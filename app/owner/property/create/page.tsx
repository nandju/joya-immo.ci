        ...
        <div className="bg-white rounded-xl shadow-md p-6 sm:p-8 border border-gray-200">
          ...
          {currentStep === 3 && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { key: "ecoleProximite", label: "École à proximité" },
                  { key: "facileAcces", label: "Facile d'accès" },
                  { key: "marcheProximite", label: "Marché à proximité" },
                  { key: "boulangerieProximite", label: "Boulangerie à proximité" },
                  { key: "parcProximite", label: "Parc à proximité" },
                  { key: "parking", label: "Parking disponible" },
                  { key: "internetRapide", label: "Accès internet rapide" },
                  { key: "chauffageCentral", label: "Chauffage central" },
                  { key: "climatisation", label: "Climatisation" },
                  { key: "piscine", label: "Piscine" },
                  { key: "jardin", label: "Jardin" },
                  { key: "securite", label: "Sécurité (gardien/alarme)" }
                ].map(({ key, label }) => (
                  <label key={key} className="flex items-center space-x-3 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={formData.equipements[key as keyof typeof formData.equipements]}
                      onChange={(e) => setFormData({
                        ...formData,
                        equipements: {...formData.equipements, [key]: e.target.checked}
                      })}
                      className="w-5 h-5 text-[#252525] border-gray-300 rounded focus:ring-[#EADD8E]"
                    />
